...
A useful generalization: a Linux host without user-namespace support (hardened kernels, some containers) degrades to `LoudBestEffort` instead of failing or silently pretending — the grade machinery covers every "sandbox weaker than advertised" situation uniformly, not just macOS.
...
A configured target is `(target, Configuration)`; configuration is folded into analysis memos, action keys, and artifact identity. Two built-in axes — `ExecMode { Incremental, Hermetic }` and `BuildProfile { Dev, Release }` — with **per-rule interpretation** (each rule decides what `Hermetic` means for its tool). Deliberately absent: user-defined transitions, `select()`. Bazel's configuration complexity radiates overwhelmingly from transitions making configuration *a function of the path you arrived by*; Anneal's restriction keeps it *a function of the node*, and that single property is the firewall.
