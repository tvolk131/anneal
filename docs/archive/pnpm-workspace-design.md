# `pnpm_workspace` — archived scope and design

> **Archived:** This file preserves the Milestone 1 pnpm rule design and deferred alternatives.
> It is not the current rule reference. See [`../pnpm-workspace.md`](../pnpm-workspace.md), the
> [README status matrix](../../README.md#current-implementation-status), and the current
...
| Current | Deferred |
|---|---|
| Frozen offline installation into managed phase-separated state | External registry acquisition and portable pnpm store |
| Explicitly declared test/build scripts | Lifecycle and native-build actions |
| Plain-path generated-file routing | Generated-package name resolution |
| Nix-manifest-provided Node, pnpm, and runtime closure | User-facing toolchain registration/provisioning |
| Exit-based script success/failure | Structured JavaScript test results and flake policy |
...
- Axis mapping per §7; Node, pnpm, and the POSIX runtime are resolved through the
  closure-complete `ANNEAL_TOOLCHAIN_MANIFEST`. Ambient `PATH` discovery is not used by the
  first-party rule.
...
- User-facing toolchain registration/provisioning for Node and pnpm, shared with Cargo.
