# What a rule is — archived design

> **Archived:** This is the original long-form rule/trust derivation and reproducibility
> graduation proposal. It is not the current rule contract. See
> [`../rules.md`](../rules.md) and [`../../TODO.md`](../../TODO.md).
...
- It **cannot see ordinary generated artifacts.** A downstream action's output does not exist
  ...
  wall: automatically consuming a generated `Cargo.toml` / `pnpm-workspace.yaml` to reshape
  analysis needs a staged-graph mechanism. An explicit `anneal materialize` followed by a new
  invocation is available today, while routing a generated `config.json` directly into an
  execution action is fine.
...
- **`providers` — the interface offered upward.** What this target exposes to anything that
  ...
  `LibraryInfo`, …). Providers flow *up*, configuration flows *down*. Routing a generated
  artifact across a language boundary is entirely a
...
> **Known implementation deviation:** ordinary action shapes currently default to a
> deterministic cache policy, and `genrule` lowers arbitrary `sh -c` commands without reducing
> that policy. The verification/graduation system described in this section is not yet the
> executor's complete behavior. Until the implementation is reconciled, generic-command
> cacheability must be treated as partial rather than as an earned guarantee. This is a code
> defect against the intended contract, not permission to weaken the contract silently.
...
decompose" — the only way to *earn* a structural guarantee for
...
- **Contained** — snapshot owners are capped at the `Local` cache tier
  ...
  future shared/remote cache. A neutrality violation is therefore contained to local state;
  the current product has no remote backend through which it could reach a team namespace.
...
| Output is reproducible | **Target: verified; current generic path: not yet enforced** | proposed off-hot-path double-build byte-compare, one-sided error (§4) | Until implemented, arbitrary-command caching is a known correctness gap |
| Result is cacheable / its cache tier | **Target: derived; current built-ins set policy during lowering** | engine policy plus enforcement and provenance | A rule-policy error can currently cache an unsafe generic action; tracked as Priority 0 |
...
6. **What is cacheable, and how is that *earned*?** The safe target contract defaults to
   non-cacheable and grants reuse only through an enforced rule/system policy. Until the
   reproducibility-graduation design in §4 is implemented, audit the lowering of every built-in
   action explicitly and do not infer that `Sealed` means deterministic.
