...
**Snapshot key:** `(platform, node major, pnpm major, pnpm-lock.yaml digest)`. Platform is
**mandatory** in the key — pnpm installs only the `os`/`cpu`/`libc`-matching variant of
platform-specific `optionalDependencies` (e.g. `@esbuild/darwin-arm64` vs `@esbuild/linux-x64`),
so `node_modules` content legitimately differs per platform.
...
hardlinks from a content-addressed store plus *relative* symlinks. The historical
non-determinism source — dependency lifecycle/`postinstall` scripts (native builds: bcrypt,
Prisma, esbuild) — is, as of **pnpm 10, blocked by default** and must be allowlisted via
`onlyBuiltDependencies`. So the leaky surface is small and *declared*, fitting the
sealed-or-uncached posture for lifecycle scripts.
...
- `install` action: resolve + install, cached + snapshot (`node_modules`, store).
...
5. "Deterministic install" is precisely **resolution**; lifecycle scripts are in the
   sealed-or-uncached bucket.
